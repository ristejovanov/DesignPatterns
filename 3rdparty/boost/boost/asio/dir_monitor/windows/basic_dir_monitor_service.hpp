...
namespace helper {
        ...
        static inline void throw_system_error_if(bool condition, const std::string& msg)
        {
            if (condition)
            {
        ...
        static inline std::string to_utf8(WCHAR *filename, DWORD length)
        {
...
public:
    ...
    explicit basic_dir_monitor_service(boost::asio::io_service &io_service)
        ...
        async_monitor_thread_(&boost::asio::basic_dir_monitor_service<DirMonitorImplementation>::io_service_work, this)
    {
    }
...
private:
    ...
    {
      DirMonitorImplementation::at_thread_start();
